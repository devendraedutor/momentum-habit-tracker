  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in select-none">
      ...
      <motion.div
        ...
        className="relative z-10 max-w-3xl w-full mx-auto p-5 sm:p-7 bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden max-h-[92vh] flex flex-col"
      >
        {/* Ambient Top Glow */}
        ...
        <div className="pt-3 pb-2.5 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 relative z-20 space-y-2.5">
          ...
          <div className="flex items-center justify-between gap-3 w-full flex-wrap sm:flex-nowrap">
            ...
            <div className="flex items-center gap-2 ml-auto">
              ...
              <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800/80 p-0.5 sm:p-1 rounded-2xl border border-slate-200/60 dark:border-slate-700/60">
                {RANGE_OPTIONS.map((range) => {
                  ...
                  return (
                    <button
                      ...
                      className={`font-mono text-[11px] sm:text-xs px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-xl transition-all cursor-pointer font-bold ${
                        isActive
              ...
              <div className="flex items-center gap-1">
                ...
                {weekOffset !== 0 && (
                  <button
                    ...
                    className="px-2 sm:px-2.5 py-1 rounded-xl bg-amber-500/10 text-amber-600 dark:text-amber-400 hover:bg-amber-500/20 text-[10px] sm:text-xs font-mono font-bold transition-colors cursor-pointer border border-amber-500/20"
                  >
                    Current
                  </button>
                )}
          ...
          <div className="grid grid-cols-7 gap-1.5 sm:gap-2 text-center px-0.5">
            {WEEKDAYS.map((dayName) => (
              <div
                key={dayName}
                className="text-[11px] font-bold text-slate-400 dark:text-slate-500 font-mono tracking-widest text-center uppercase"
              >
                {dayName}
              </div>
            ))}
        ...
        <div className="overflow-y-auto max-h-[55vh] pr-1 pt-2 pb-2 scrollbar-thin">
            ...
            {days.map((day) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-xl border p-1.5 sm:p-2 flex flex-col justify-between aspect-[1/0.95] sm:aspect-square transition-all cursor-pointer select-none text-center hover:scale-[1.04] active:scale-95 ${tileStyle} ${
                ...
                >
                  ...
                  <div className="flex items-center justify-between w-full px-0.5 text-[9px] font-mono leading-none">
                    <span className="font-mono text-[11px] sm:text-xs font-black">{day.dayNum}</span>
                  ...
                  <div className="my-auto flex items-center justify-center">
                    {isConquered ? (
                      <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-gradient-to-b from-amber-300 to-amber-500 text-white flex items-center justify-center shadow-sm shadow-amber-500/40">
                        <Gem className="w-3 h-3 sm:w-3.5 sm:h-3.5 fill-white text-white drop-shadow-xs" />
                      </div>
                    ) : isMissed ? (
                      <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-rose-500 text-white flex items-center justify-center text-[10px] sm:text-xs font-black shadow-sm shadow-rose-500/20">
