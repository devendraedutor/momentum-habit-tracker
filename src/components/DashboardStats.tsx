...
import { Flame, Award, Zap, TrendingUp, TrendingDown, Gem, Lock } from 'lucide-react';
...
export const DashboardStats: React.FC<DashboardStatsProps> = ({
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-3">
      ...
      {/* 2. Jumbo Points (💎 100% Perfect Days - Unlocked at >= 3 habits) */}
      <div className={`app-card rounded-2xl p-3.5 sm:p-4 flex flex-col justify-between ${
        activeHabits.length >= 3
          ? 'bg-gradient-to-br from-amber-500/5 via-transparent to-yellow-500/5 border-amber-500/20'
          : 'bg-slate-50 dark:bg-slate-800/40 border-slate-200 dark:border-slate-800 opacity-80'
      }`}>
        <div className="flex items-center justify-between">
          <span className={`text-[10px] sm:text-xs font-semibold uppercase tracking-wider ${
            activeHabits.length >= 3 ? 'text-amber-600 dark:text-amber-400' : 'text-slate-500'
          }`}>
            Jumbo Points
          </span>
          <div className={`p-1.5 rounded-lg ${
            activeHabits.length >= 3 ? 'bg-amber-500/15 text-amber-500' : 'bg-slate-200 dark:bg-slate-700 text-slate-400'
          }`}>
            {activeHabits.length >= 3 ? <Gem className="w-3.5 h-3.5 fill-amber-400" /> : <Lock className="w-3.5 h-3.5" />}
          </div>
        </div>
        <div className="mt-2 flex items-baseline gap-1">
          {activeHabits.length >= 3 ? (
        ...
        <div className={`mt-1 text-[11px] font-mono ${
          activeHabits.length >= 3 ? 'text-amber-600 dark:text-amber-400 font-medium' : 'text-slate-400'
        }`}>
          {activeHabits.length >= 3 ? '💎 100% Perfect Days' : 'Requires ≥3 active habits'}
        </div>
      </div>
